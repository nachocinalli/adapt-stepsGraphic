...
export default function StepsGraphic(props) {
  const { _graphic, _hasDescriptionBelowNavigation, _items, onStepClick, onControlClick } = props;
  let _activeIndex = 0;
    ...
    <div className='component__inner stepsgraphic__inner'>
      ...
      <div className='stepsgraphic__controls'>
        <button
          ...
          data-direction='left'
          className={classes(['btn-icon', 'stepsgraphic__controls-left', _activeIndex === 0 && 'is-disabled'])}
          aria-label=''
        ...
        <button
          ...
          className={classes(['btn-icon', 'stepsgraphic__controls-right', _activeIndex === _items.length - 1 && 'is-disabled'])}
          aria-label=''
      ...
      <div className={classes(['component__widget', 'stepsgraphic__widget'])}>
        <div className={classes(['stepsgraphic__graphic-intro'])}>
          <templates.image
            {..._graphic}
        ...
        <div className={classes(['stepsgraphic__item-graphic-container'])}>
          {_items.map(({ title, body, _graphic, _classes, _index, _isVisited, _hasPin, _pinTop, _pinLeft, _pinClass, _isNumeric, _isActive }) => (
            <div
              className={classes(['stepsgraphic__item', _isVisited && 'is-visited', _isActive && 'is-active', _classes])}
              key={_index}
              data-index={_index}
            >
              <templates.image
              ...
              {_hasPin && (
                ...
                >
                  {_isNumeric && <div dangerouslySetInnerHTML={{ __html: compile(`${_index + 1}`) }}></div>}
                </div>
              )}
              {!_hasDescriptionBelowNavigation && (
                <div className='stepsgraphic__item-description'>
                  <div className='stepsgraphic__item-title' dangerouslySetInnerHTML={{ __html: compile(title) }}></div>
                  <div className='stepsgraphic__item-body' dangerouslySetInnerHTML={{ __html: compile(body) }}></div>
                </div>
              )}
            </div>
          ))}
        ...
        <div className='stepsgraphic__nav'>
          {_items.map(({ title, _index, _isVisited, _isActive }) => (
            <button
              onClick={onStepClick}
              key={_index}
              className={classes(['btn-text', 'js-stepgraphic-item', _isVisited && 'is-visited', _isActive && 'is-active'])}
              data-index={_index}
        ...
        {_hasDescriptionBelowNavigation && (
          <div className='stepsgraphic__item-container stepsgraphic__break'>
